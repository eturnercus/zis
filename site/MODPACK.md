...
Ядро сервера: **Minecraft 1.21.1 / NeoForge 21.1.248**
...
4. **C2ME** снят: у ядра свои потоки сущностей. Параллельная генерация чанков с гибридным chunk map уже кусалась. Не включаем `parallel-vanilla-worlds`.
5. **Responsive Shields 2.4** — mixin в tick-const щита даёт `InjectionError`. Снят в `mods-disabled`.
...
`auto_delete_mods` выключен. Ядро не имеет права само выкидывать банки.
...
| ~~responsiveshields~~ | Снят: mixin `setShieldUseDelay` не цепляется (нет refMap). В `mods-disabled` |
...
| Lagshield / memguard / evolution | Щиты по памяти/лагу. Слой пересекается с AllTheLeaks — не плодить ещё | сервер |
...
1. Сервер: `server\start.bat` (Windows) или `start.sh` (Linux). Ядро лежит в папке `server`.
2. JVM: `user_jvm_args.txt`. На 32 ГБ хосте поднимайте `-Xmx` до 18G как на старом NoteBuns-старте. На слабее — 8–12G.
3. Потоки сущностей включены, `parallel-vanilla-worlds: false`.
4. Клиент только через `Launcher`. Смешивать Sodium в `server/mods` нельзя.
5. Первый запуск ядра качает libraries. Это норма, не краш.
...
- **Responsive Shields** — снят с dedicated: inject в константу задержки щита ломается. Better Combat остаётся.
